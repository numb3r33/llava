...
# %% auto 0
__all__ = ['llava_stage1_splitter', 'get_stage1_learner', 'train_stage1']
...
if project_root_str not in sys.path:
    ...
    # This message might be less relevant when running as a script
...
# Attempt to import from the llava library (assuming nbdev_export has run)
try:
...
# %% ../nbs/31_training_stage1.ipynb 5
...
# %% ../nbs/31_training_stage1.ipynb 10
...
if __name__ == "__main__" and "get_ipython" not in locals():
    ...
    # Default path assumes the script is run from the project root (e.g., python scripts/run_train_stage1.py)
    parser.add_argument("--config", type=str, default="configs/config.yaml", 
                        help="Path to the configuration YAML file (relative to project root or absolute).")
    args = parser.parse_args()
    
    # Construct absolute path to config file relative to project root
    config_file_path = project_root / args.config 
    
    if not config_file_path.is_file():
        print(f"Error: Config file not found at {config_file_path}")
        print(f"(Tried resolving from project root: {project_root})")
        sys.exit(1)

    try:
        train_stage1(config_path=config_file_path)
    except Exception as e:
